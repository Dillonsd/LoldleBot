...
function buildTrollAnswer(answer: Champion, gameId: number): Champion {
  // Use gameId as a stable seed so the fake answer is consistent across all guesses in a game
  const pick = <T>(arr: T[]) => arr[gameId % arr.length];

  // Wrong year: any year in the pool except the real one
  const yearPool = [2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020,2021,2022,2023,2024]
    .filter((y) => y !== answer.releaseYear);
  const fakeYear = pick(yearPool);

  return {
    ...answer,
    releaseYear: fakeYear,
    region: "None",
    species: ["None"],
    // gender, positions, resource, rangeType kept from real answer
  };
}
...
export async function execute(
  ...
  const effectiveAnswer = trolled
    ? buildTrollAnswer(answer, game.id)
    : answer;
